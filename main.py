...
def convert_to_ut(date_str: str, time_str: str, timezone_str: str):
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Erro ao converter data/hora: {str(e)}"
        )
...
def calculate_map(birth_data: BirthData) -> MapResult:
    ...
    for i, cusp in enumerate(houses, start=1):
        ...
        deg = cusp % 30
        house_list.append(HouseInfo(house=i, sign=sign, degree=round(deg, 2)))
    positions = []
    for planet_name, code in PLANETS_SWEPH.items():
        ...
        positions.append(PlanetPosition(
            planet=planet_name, sign=sign, degree=round(deg, 2),
            house=planet_house, retrograde=is_retro
        ))
    ...
    if nodo_norte:
        ...
        positions.append(PlanetPosition(
            planet="NóduloSul", sign=south_sign,
            degree=round(south_deg_in_sign, 2),
            house=nodo_sul_house, retrograde=False
        ))
    asc_long = asc_mc[0]
    ...
    asc_deg = asc_long % 30
    positions.append(PlanetPosition(
        planet="Ascendente", sign=asc_sign, degree=round(asc_deg, 2), house=1, retrograde=False
    ))
    mc_long = asc_mc[1]
    ...
    mc_deg = mc_long % 30
    positions.append(PlanetPosition(
        planet="MeioCéu", sign=mc_sign, degree=round(mc_deg, 2), house=10, retrograde=False
    ))
