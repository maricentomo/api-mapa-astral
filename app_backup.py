...
if "etapa" not in st.session_state:
    st.session_state.etapa = "inicial"  # Pode ser: inicial, chatbot_ativo
...
with st.sidebar:
    ...
    # Adicionar menu de seleção para diferentes robôs
    robo_selecionado = st.radio(
        "Escolha um assistente:",
        ["🔮 Astro IA", "🧠 Coach IA", "💭 Tarot IA", "✨ Numerologia IA"],
        index=0
    )

    st.divider()

    # Mostrar informações do robô selecionado
    if robo_selecionado == "🔮 Astro IA":
        st.subheader("🔮 Astro IA")
        st.write("Interpretação completa de mapas astrais")
    elif robo_selecionado == "🧠 Coach IA":
        st.subheader("🧠 Coach IA")
        st.write("Seu assistente para desenvolvimento pessoal")
    elif robo_selecionado == "💭 Tarot IA":
        st.subheader("💭 Tarot IA")
        st.write("Leitura e interpretação de tarot")
    elif robo_selecionado == "✨ Numerologia IA":
        st.subheader("✨ Numerologia IA")
        st.write("Análise numerológica personalizada")

    # API key já configurada no código
    st.divider()
    st.success("Conectado ao Cérebro Gemini! 🧠")
    st.info("🔹 Motor de Cálculo: Local (Porta 8000)")
...
if st.session_state.etapa == "inicial":
    ...
    if submit:
        ...
        try:
            # 2. CHAMAR O MOTOR DE CÁLCULO (LOCAL)
